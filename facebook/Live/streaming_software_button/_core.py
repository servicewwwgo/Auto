"""
facebook.com/Live/streaming_software_button 元素实现

Element 子类，表示直播页的软件流按钮。
参考 autojs.py 中 streaming_software_button 定义。
"""
...
class StreamingSoftwareButton(Element):
    """Facebook 直播页软件流按钮。"""

    def __init__(self, browser: Browser, node_name: str, domain: Domain, page: Page, language: str = "en-US"):
        super().__init__(browser=browser, node_name=node_name, domain=domain, page=page, description="Live.软件流按钮", language=language)
        self._element = ElementClass(
            tab_id=self.domain.tab_id,
            name="streaming_software_button",
